...
function FormConfigEditor({ config, onSave }) {
  ...
  const handleAddOption = () => {
    ...
    if (!trimmed) { toast.error('Option cannot be empty'); return }
  ...
  const handleDeleteOption = (index) => {
    if (options.length <= 1) { toast.error('Must have at least one option'); return }
    setOptions(options.filter((_, i) => i !== index))
  }

  const handleSave = async () => {
    if (!label.trim()) { toast.error('Label cannot be empty'); return }
    if (options.length === 0) { toast.error('Must have at least one option'); return }
    setSaving(true)
    try {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 md:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
            <div>
              <p className="text-gray-600 bg-indigo-50 px-4 py-2 rounded-lg border border-indigo-100 inline-block text-sm md:text-base font-medium">
                Configure dynamic fields for each step of the RRF workflow
              </p>
            </div>
            <button
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-200">
            {steps.map((step, index) => (
              <div key={step.number} className="flex items-center">
                ...
                {index < steps.length - 1 && <RightOutlined className="mx-2 sm:mx-4 text-gray-400" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mt-6 md:mt-8 p-4 md:p-6 bg-blue-50 border border-blue-200 rounded-xl">
          ...
          <ul className="text-sm text-blue-800 space-y-1">
            ...
            <li>• SubFunction options are shared across all Functions</li>
