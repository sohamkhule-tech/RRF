...
export default function PMOClosedPage() {
  ...
  const closedCount = closedRequests.length
    ...
    <div className="p-4 md:p-8 space-y-8">
      ...
      {loading ? (
        ...
        <div className="flex gap-4">
          <div className="flex items-center gap-3 px-3 py-2 md:px-4 md:py-2.5 bg-white border-2 border-gray-200 rounded-xl shadow-md">
            <div className="text-xl">🔒</div>
            <div>
              <p className="text-sm text-gray-700 font-bold">Closed</p>
              <p className="text-lg md:text-2xl font-bold text-gray-600">{closedCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-4">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="flex-1 relative group">
            <input
              type="text"
              placeholder="Search by RRF ID, Role, Manager, Project, Sub Function, or Candidate..."
              value={searchTerm}
      ...
      <div className="bg-white overflow-hidden" style={{ borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.08)', padding: '20px' }}>
        <div className="px-2 py-4 mb-4">
          ...
          <p className="text-sm text-gray-500 mt-1">RRFs that have been completely closed out</p>
        </div>
              ...
              {filteredRequests.length > 0 ? (
                filteredRequests.map((request) => (
                  <tr key={request.rrfId} className="hover:bg-gray-50 transition-colors duration-200">
                    ...
                    <td className="px-3 py-3 whitespace-nowrap text-xs text-gray-600">{request.subFunction}</td>
                ...
                <tr>
                  <td colSpan={9} className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center space-y-3">
                      ...
                      <p className="text-lg font-medium text-gray-900">No Closed RRFs Found</p>
                      <p className="text-sm text-gray-500 max-w-sm text-center">
                        There are currently no RRFs in the "Closed" stage. Once an open request is closed, it will appear here.
                      </p>
                    </div>
                  </td>
                </tr>
        ...
        <div className="md:hidden px-2 pb-4 space-y-3">
          {filteredRequests.length > 0 ? (
              ...
              <div key={request.rrfId} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 truncate">{request.project} · {request.subFunction}</div>
                  </div>
                </div>
            ...
            <div className="py-8 text-center text-gray-500">
              <p className="text-sm font-medium">No closed requests found</p>
            </div>
          )}
        </div>
      </div>
