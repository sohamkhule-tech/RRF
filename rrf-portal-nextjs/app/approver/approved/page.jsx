...
export default function ApproverApprovedPage() {
  ...
  const { requests, loading, error, refresh } = useApproverRequests('approved', { limit: 1000 })
  ...
  if (loading) {
    return (
      <ProtectedRoute requiredPermission={PERMISSIONS.APPROVALS.READ}>
        <div className="p-8">
          <LoadingSpinner message="Loading approved requests..." />
        </div>
      </ProtectedRoute>
    )
  }
  ...
  return (
    <ProtectedRoute requiredPermission={PERMISSIONS.APPROVALS.READ}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <p className="text-lg text-gray-800 font-bold">RRF requests you have approved and sent to PMO</p>
        </div>
        <div className="flex items-center gap-3 px-3 py-2 md:px-5 md:py-3 bg-white border-2 border-green-200 rounded-xl shadow-md">
          <div className="text-2xl">✅</div>
          <div>
            <p className="text-sm text-gray-700 font-bold">Total Approved</p>
            <p className="text-xl md:text-2xl font-bold text-green-600">{approvedRequests.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-4">
        <div className="flex flex-col md:flex-row gap-3 md:gap-4">
          ...
          <div className="flex-1 relative group">
            <input
              type="text"
              placeholder="Search by RRF ID, Manager Name, Role, Department, or Project..."
              value={searchTerm}
      ...
      <div className="bg-white overflow-hidden" style={{ borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.08)', padding: '20px' }}>
        ...
        <div className="px-2 py-4 mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b-2 border-gray-100">
          <div>
            <h3 className="text-lg font-bold text-gray-900">Approved RRF Requests</h3>
            <p className="text-sm text-gray-500 mt-1">RRF requests you have reviewed and approved</p>
          </div>
        </div>
              ...
              <div key={request.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500 truncate">{request.subFunction || '-'} • {request.projectName || 'N/A'}</div>
                  </div>
