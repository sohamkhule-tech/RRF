 ...
 * Rollback: remove the redirect and restore `export default` on LegacyApproverClosedPage.
 */
...
import { EyeOutlined, LeftOutlined, CheckCircleOutlined } from '@ant-design/icons'
...
function LegacyApproverClosedPage() {
  const router = useRouter()
  const [searchTerm, setSearchTerm] = useState('')
  // ✅ REMOVED: selectedDepartment filter
  
  // Fetch closed RRFs from backend
  const { requests, loading, error, refresh } = useApproverRequests('closed,closed-by-bench', { limit: 1000 })
  ...
  const getStatusBadge = (status) => {
    ...
    if (s === 'closed-by-bench' || s === 'closed_by_bench') {
      return (
        <span className="text-xs font-medium bg-indigo-100 text-indigo-700 rounded-full px-3 py-1.5 border border-indigo-200">
          Filled from Bench
        </span>
      )
    }
    return (
      <span className="text-xs font-medium bg-green-100 text-green-700 rounded-full px-3 py-1.5 border border-green-200">
        Position Filled
      </span>
    )
  }
  ...
  const filteredRequests = closedRequests.filter(request => {
    // ✅ REMOVED: Department filter dropdown
    
    // Search filter
    if (!searchTerm) return true
    const searchLower = searchTerm.toLowerCase()
    const rrfId = (request.rrfNumber || request.subId || `RRF-${request.id}`).toLowerCase()
    return (
      rrfId.includes(searchLower) ||
  ...
  if (loading) {
    return (
      <ProtectedRoute requiredPermission={PERMISSIONS.APPROVALS.READ}>
        <div className="p-8">
          <LoadingSpinner message="Loading closed requests..." />
        </div>
      </ProtectedRoute>
    )
  }
  ...
  return (
    <ProtectedRoute requiredPermission={PERMISSIONS.APPROVALS.READ}>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Closed Requests</h1>
          <p className="text-lg text-gray-600 font-medium">Historical records of completed requisitions</p>
        </div>
        <div className="flex items-center gap-3 px-3 py-2 md:px-5 md:py-3 bg-white border-2 border-indigo-200 rounded-xl shadow-md">
          <div className="text-2xl">🏁</div>
          <div>
            <p className="text-sm text-gray-700 font-bold">Total Closed</p>
            <p className="text-xl md:text-2xl font-bold text-indigo-600">{closedRequests.length}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-4">
        <div className="flex flex-col md:flex-row gap-3 md:gap-4">
          ...
          <div className="flex-1 relative group">
            <input
              type="text"
              placeholder="Search by ID, Manager, Role, Project, or Candidate..."
              value={searchTerm}
      ...
      <div className="bg-white overflow-hidden" style={{ borderRadius: '16px', boxShadow: '0 10px 25px rgba(0,0,0,0.08)', padding: '20px' }}>
        ...
        <div className="px-2 py-4 mb-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 border-b-2 border-gray-100">
          <div>
            <h3 className="text-lg font-bold text-gray-900">Completed Requisitions</h3>
            <p className="text-sm text-gray-500 mt-1">Details of filled and closed positions</p>
          </div>
        </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="px-3 py-3 text-left text-xs font-bold text-gray-700 uppercase">Closure Details</th>
              ...
              {filteredRequests.map((request) => (
                <tr key={request.id} className="border-b border-gray-100 transition-all duration-200 hover:bg-gray-50">
                  <td className="px-3 py-3 whitespace-nowrap text-xs font-bold text-indigo-600">
                    {request.rrfNumber || request.subId}
                  ...
                  <td className="px-3 py-3 text-xs">{getStatusBadge(request.status)}</td>
                  <td className="px-3 py-3 text-xs">
                    <div className="font-bold text-gray-900">{request.candidateName || request.notes || 'N/A'}</div>
            ...
            <div key={request.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-500 truncate">{request.projectName || 'N/A'}</div>
                </div>
              ...
              <div className="text-xs text-gray-600 mb-3 p-2 bg-gray-50 rounded-lg">
                <span className="font-bold">Candidate:</span> {request.candidateName || 'N/A'}<br/>
        ...
        {filteredRequests.length === 0 && (
          <div className="py-12 text-center text-gray-500">
            <p className="text-lg font-medium">No closed requests found</p>
            <p className="text-sm">Requests closed by HR or PMO will appear here.</p>
          </div>
        )}
      </div>
    </div>
