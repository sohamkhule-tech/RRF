'use client'

/**
 * /requests/[id] — Unified ViewRRF Page (Phase 1)
 *
 * A single universal route that serves all roles (HM, Approver, PMO, HR, Admin).
 * Which action buttons appear is determined by resolveActions() — a pure function
 * that evaluates role, permissions, ownership, and RRF status.
 *
 * Constraints (Phase 1):
 *   - Old role-specific ViewRRF routes are NOT modified (Strangler Pattern)
 *   - No backend changes
 *   - No sidebar or table link changes
 *   - All changes are purely additive
 */

import { useParams, useRouter } from 'next/navigation'
import { useState } from 'react'
import {
  BuildOutlined,
  UserOutlined,
  ToolOutlined,
  FileTextOutlined,
  ArrowLeftOutlined,
} from '@ant-design/icons'
import jsPDF from 'jspdf'
import html2canvas from 'html2canvas'
import toast from 'react-hot-toast'

import { useAuth }        from '@/contexts/AuthContext'
import { useRRFDetail }   from '@/hooks/useRRFDetail'
import { rrfApi }         from '@/lib/api/rrfApi'

import { LoadingSpinner }     from '@/components/LoadingSpinner'
import { ErrorMessage }       from '@/components/ErrorMessage'
import StatusWithDetails      from '@/components/StatusWithDetails'
import RRFContentSections     from '@/components/RRFContentSections'
import { ActionButtonBar }    from '@/components/rrf/ActionButtonBar'
import { ActionModal }        from '@/components/rrf/ActionModal'
import { resolveActions }     from '@/utils/rrfActionResolver'

// Section definitions — identical to all existing ViewRRF pages
const SECTIONS = [
  { id: 'requisition', label: 'Requisition Info',      icon: <BuildOutlined    className="text-lg" /> },
  { id: 'position',    label: 'Position Details',       icon: <UserOutlined     className="text-lg" /> },
  { id: 'technical',   label: 'Technical Requirements', icon: <ToolOutlined     className="text-lg" /> },
  { id: 'description', label: 'Job Description',        icon: <FileTextOutlined className="text-lg" /> },
]

/**
 * Map raw RRF API data into the display-ready shape used by all ViewRRF pages.
 * Mirrors the exact field mapping in the existing HM / Approver pages.
 */
function buildRrfData(rrf) {
  if (!rrf) return null

  return {
    id:          rrf.id,
    displayId:   rrf.rrfNumber || rrf.subId,
    submittedDate: (rrf.submittedAt || rrf.createdAt)
      ? new Date(rrf.submittedAt || rrf.createdAt).toLocaleDateString('en-GB')
      : 'N/A',

    // Normalise status label — uppercase for consistency across the UI
    status:
      rrf.status === 'pending'         ? 'PENDING'
      : rrf.status === 'approved'      ? 'APPROVED'
      : rrf.status === 'rejected'      ? 'DECLINED'
      : rrf.status === 'declined'      ? 'DECLINED'
      : rrf.status === 'on-hold'       ? 'ON-HOLD'
      : rrf.status === 'draft'         ? 'DRAFT'
      : rrf.status === 'in-progress'   ? 'IN-PROGRESS'
      : rrf.status === 'open-for-hiring' ? 'OPEN-FOR-HIRING'
      : rrf.status === 'closed'        ? 'CLOSED'
      : String(rrf.status || '').toUpperCase(),

    managerName:            rrf.createdBy?.fullName || 'Unknown',
    entity:                 rrf.entity,
    organisation:           rrf.organisation,
    function:               rrf.function,
    subFunction:            rrf.subFunction,
    department:             rrf.department,
    requisitionType:        rrf.requisitionType,
    customerName:           rrf.customerName,
    projectName:            rrf.projectName,
    jobTitle:               rrf.positionTitle,
    billingStartDate:       rrf.billingStartDate,
    expectedOnboardingDate: rrf.expectedOnboardingDate,
    billingRate:            rrf.billingRate,
    billingCurrency:        rrf.billingCurrency,
    positionType:           rrf.positionType,
    employmentType:         rrf.employmentType,
    workMode:               rrf.workMode,
    numberOfPositions:      rrf.headcount,
    priorityLevel:          rrf.priority,
    jobLocation:            rrf.location ? [rrf.location] : null,
    minimumExperience:      rrf.experienceMin && rrf.experienceMax
                              ? `${rrf.experienceMin}-${rrf.experienceMax} years`
                              : null,
    primaryTechnologies:    rrf.technologies,
    requiredSkills:         rrf.requiredSkills,
    preferredSkills:        rrf.preferredSkills,
    jobDescription:         rrf.jobDescription,
    additionalNotes:        rrf.notes || rrf.urgencyReason,
    budgetMin:              rrf.budgetMin,
    budgetMax:              rrf.budgetMax,
    interviewers:           rrf.interviewers || [],
    approvers:              rrf.approvers    || [],
    statusHistory:          rrf.statusHistory || [],

    // Decline/hold reason — mine from multiple sources (mirrors HM page forensic logic)
    declineReason:
      rrf.declineReason ||
      rrf.notes ||
      rrf.statusHistory?.slice().reverse()
        .find((e) => ['declined', 'rejected', 'on-hold'].includes(e.status))
        ?.reason ||
      null,

    declinedBy: rrf.declinedBy?.fullName || null,

    declinedAt: (() => {
      const ts =
        rrf.declinedAt ||
        rrf.rejectedAt ||
        rrf.statusHistory?.slice().reverse()
          .find((e) => ['declined', 'rejected', 'on-hold'].includes(e.status))
          ?.changedAt
      return ts ? new Date(ts).toLocaleString('en-GB') : null
    })(),
  }
}

export default function UnifiedViewRRFPage() {
  const params = useParams()
  const router = useRouter()
  const rrfId  = params.id

  const { user }                           = useAuth()
  const { rrf, loading, error, refresh }   = useRRFDetail(rrfId)

  const [activeSection, setActiveSection] = useState('requisition')
  const [showModal,     setShowModal]     = useState(false)
  const [modalType,     setModalType]     = useState('')

  // ── Loading / Error early returns ─────────────────────────────────────────

  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 p-8">
        <LoadingSpinner message="Loading RRF details..." />
      </div>
    )
  }

  if (error || !rrf) {
    return (
      <div className="min-h-screen bg-slate-50 p-8">
        <ErrorMessage message={error || 'RRF not found'} onRetry={refresh} />
      </div>
    )
  }

  // ── Derived state (only reached when data is available) ───────────────────

  const rrfData    = buildRrfData(rrf)
  const roleCode   = String(user?.role?.code || '').toUpperCase()
  const permissions = user?.permissions || []
  const actions    = resolveActions(rrf, user, permissions)

  // ── Action handlers ───────────────────────────────────────────────────────

  const handleAction = (actionType) => {
    if (actionType === 'print') {
      window.print()
      return
    }
    if (actionType === 'exportPdf') {
      handleExportPdf()
      return
    }
    // Edit: route to the role-appropriate edit page (proper field mapping + prefill)
    if (actionType === 'edit') {
      const editPath = roleCode === 'APPROVER'
        ? `/approver/edit-rrf/${rrf.id}`
        : `/hiring-manager/edit-rrf/${rrf.id}`
      router.push(editPath)
      return
    }
    // All other actions open the unified ActionModal
    setModalType(actionType)
    setShowModal(true)
  }

  const handleModalSubmit = async (type, formData) => {
    let result

    try {
      if (type === 'approve') {
        result = await rrfApi.approve(rrfId, formData.reason || '')
      } else if (type === 'decline') {
        result = await rrfApi.decline(rrfId, formData.reason)
      } else if (type === 'hold') {
        result = await rrfApi.putOnHold(rrfId, formData.reason)
      } else if (type === 'submit') {
        result = await rrfApi.submit(rrfId)
      } else if (type === 'openForHiring') {
        result = await rrfApi.openForHiring(rrfId)
      } else if (type === 'close') {
        result = await rrfApi.close(rrfId, formData)
      }

      if (result?.success) {
        toast.success(result.message || 'Action completed successfully')
        setShowModal(false)
        refresh() // Refresh RRF data to reflect new status
      } else {
        throw new Error(result?.message || 'Action failed')
      }
    } catch (err) {
      toast.error(err.message || 'Action failed. Please try again.')
      throw err // Re-throw so ActionModal keeps itself open for retry
    }
  }

  const handleExportPdf = async () => {
    const mainContainer = document.querySelector('.min-h-screen')
    const content       = document.querySelector('.rrf-content-area')
    if (!content || !mainContainer) return

    const originalOverflow = content.style.overflow
    mainContainer.classList.add('exporting-pdf')
    content.style.overflow = 'visible'
    content.style.height   = 'auto'

    await new Promise((resolve) => setTimeout(resolve, 300))

    const canvas = await html2canvas(content, {
      scale: 2, logging: false, useCORS: true,
      backgroundColor: '#ffffff',
      height: content.scrollHeight, windowHeight: content.scrollHeight,
    })

    mainContainer.classList.remove('exporting-pdf')
    content.style.overflow = originalOverflow
    content.style.height   = ''

    const imgData  = canvas.toDataURL('image/png')
    const pdf      = new jsPDF('p', 'mm', 'a4')
    const pdfWidth = pdf.internal.pageSize.getWidth()
    const pdfHeight = (canvas.height * pdfWidth) / canvas.width
    let position   = 0
    const pageHeight = pdf.internal.pageSize.getHeight()

    if (pdfHeight > pageHeight) {
      while (position < pdfHeight) {
        pdf.addImage(imgData, 'PNG', 0, -position, pdfWidth, pdfHeight)
        position += pageHeight
        if (position < pdfHeight) pdf.addPage()
      }
    } else {
      pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight)
    }

    pdf.save(`${rrfData.displayId || rrfId}.pdf`)
    toast.success('PDF exported successfully!')
  }

  // ── Render ────────────────────────────────────────────────────────────────

  return (
    <div className="min-h-screen bg-slate-50">

      {/* Print & Export PDF styles — identical to all existing ViewRRF pages */}
      <style jsx global>{`
        @media screen {
          .screen-hidden { display: none !important; }
          .print-header  { display: none !important; }
          .exporting-pdf .screen-hidden { display: block !important; }
          .exporting-pdf .print-header  { display: block !important; }
          .exporting-pdf .no-print      { display: none  !important; }
        }
        @media print {
          * { -webkit-print-color-adjust: exact; print-color-adjust: exact; }
          .no-print      { display: none  !important; }
          .screen-hidden { display: block !important; }
          .print-section { display: block !important; page-break-inside: avoid; margin-bottom: 30px; }
          .print-header  { display: block !important; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid #cbd5e1; }
          body { background: white !important; }
          .rrf-content-area { overflow: visible !important; height: auto !important; }
          .flex.h-screen    { height: auto !important; }
          .overflow-y-auto  { overflow: visible !important; }
        }
      `}</style>

      <div className="flex flex-col md:flex-row h-screen">

        {/* ── LEFT PANEL — Summary sidebar ───────────────────────────────── */}
        <div className="w-full md:w-80 bg-white border-b md:border-b-0 md:border-r border-slate-200 flex flex-col md:overflow-y-auto no-print">

          {/* Header with gradient + status badge */}
          <div className="p-4 md:p-6 bg-gradient-to-br from-slate-700 to-slate-800 text-white">
            <div className="space-y-3">
              <h1 className="text-lg md:text-2xl font-bold tracking-tight leading-tight">
                {rrfData.jobTitle}
              </h1>
              <div className="mt-2">
                <StatusWithDetails
                  status={rrfData.status}
                  reason={rrfData.declineReason}
                  actionBy={rrfData.declinedBy}
                  actionDate={rrfData.declinedAt}
                />
              </div>
              <div className="text-xs text-slate-300 font-medium space-y-1">
                <div>{rrfData.displayId}</div>
                <div>Requested on: {rrfData.submittedDate}</div>
              </div>
            </div>
          </div>

          {/* Section navigation */}
          <div className="flex-1 p-3 md:p-6">
            <div className="flex md:flex-col gap-2 md:gap-0 md:space-y-1 overflow-x-auto md:overflow-x-visible pb-2 md:pb-0">
              {SECTIONS.map((section, index) => (
                <div key={section.id}>
                  <button
                    onClick={() => setActiveSection(section.id)}
                    className={`flex-shrink-0 md:flex-shrink md:w-full flex items-center gap-2 md:gap-4 p-3 md:p-4 rounded-lg transition-all duration-200 ${
                      activeSection === section.id
                        ? 'bg-gradient-to-r from-blue-600 to-blue-700 text-white shadow-lg'
                        : 'bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-200'
                    }`}
                  >
                    <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center ${
                      activeSection === section.id ? 'bg-white/20' : 'bg-slate-100'
                    }`}>
                      {section.icon}
                    </div>
                    <span className="text-xs font-bold uppercase tracking-wider whitespace-nowrap">
                      {section.label}
                    </span>
                  </button>
                  {index < SECTIONS.length - 1 && (
                    <div className="hidden md:block w-px h-4 bg-slate-300 ml-9 my-0.5" />
                  )}
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* ── RIGHT PANEL — Action bar + content ─────────────────────────── */}
        <div className="flex-1 flex flex-col overflow-hidden">

          {/* Top action bar */}
          <div className="bg-white border-b border-slate-200 px-4 md:px-8 py-3 md:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 no-print">
            <button
              onClick={() => router.back()}
              className="flex items-center gap-2 px-4 py-2 text-slate-700 hover:bg-slate-100 rounded-lg transition-all duration-200 border border-slate-300"
            >
              <ArrowLeftOutlined className="text-lg" />
              <span className="font-medium">Back</span>
            </button>

            {/* Dynamic action buttons — visibility driven by resolveActions() */}
            <ActionButtonBar actions={actions} onAction={handleAction} />
          </div>

          {/* Scrollable content area */}
          <div className="flex-1 overflow-y-auto bg-white rrf-content-area">
            <div className="p-4 md:p-8">

              {/* Print-only header (hidden on screen) */}
              <div className="print-header">
                <h1 className="text-3xl font-bold text-slate-900 mb-2">{rrfData.jobTitle}</h1>
                <div className="flex items-center gap-4 text-sm text-slate-600">
                  <span>{rrfData.displayId}</span>
                  <span>•</span>
                  <span>Submitted: {rrfData.submittedDate}</span>
                  <span>•</span>
                  <StatusWithDetails
                    status={rrfData.status}
                    reason={rrfData.declineReason}
                    actionBy={rrfData.declinedBy}
                    actionDate={rrfData.declinedAt}
                  />
                </div>
              </div>

              {/* Shared content sections — identical component used by all ViewRRF pages */}
              <RRFContentSections rrfData={rrfData} activeSection={activeSection} />
            </div>
          </div>

        </div>
      </div>

      {/* Unified Action Modal — renders correct form fields based on modalType */}
      <ActionModal
        modalType={modalType}
        isOpen={showModal}
        onClose={() => setShowModal(false)}
        onSubmit={handleModalSubmit}
        roleCode={roleCode}
      />

    </div>
  )
}
